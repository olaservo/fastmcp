...
<ParamField body="mirror_structured_content" type="bool" default="True">
  Whether a structured result is also serialized into a `content` text block (the spec's SHOULD). When `True` (default), the value is sent both as `structured_content` and as a serialized copy in `content`. Set to `False` to send `structured_content` only, without the duplicate serialized copy on the wire. See [Structured Output](#structured-output) for details.
</ParamField>
</Card>
...
<Tip>
The serialized copy in `content` is the spec's SHOULD, not a MUST — it keeps the value available to a client that reads `content`. For a large payload it is pure duplication, since the same data is already in `structured_content`. Pass `mirror_structured_content=False` to send `structured_content` only, with empty `content`:
...
Which field a client forwards to a model is the client's choice; this flag only controls whether the serialized copy is emitted.
</Tip>
